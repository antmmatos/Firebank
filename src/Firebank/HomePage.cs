...
namespace Firebank
{
    public partial class Homepage : Form
    {
        ...
        private void StartUPCards()
        {
            SqlCommand command = new SqlCommand();
            command.Connection = db;
            command.CommandText = "SELECT Cards.ID, CardNumber, CardExpireDate, CardCVV, AccountID, isActivated, isFreeze FROM Users INNER JOIN Accounts ON Users.ID = Accounts.Account_Owner INNER JOIN Cards ON Accounts.ID = Cards.AccountID";
            SqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                CardsComboBox.Items.Add("ID: " + reader.GetInt32(0).ToString());
                cards.Add(new Card(reader.GetInt32(0), reader.GetString(1), reader.GetDateTime(2), reader.GetString(3), reader.GetInt32(4), reader.GetBoolean(5), reader.GetBoolean(6)));
            }
        ...
        {
            cardsmanagementpanel.Visible = true;
        }
